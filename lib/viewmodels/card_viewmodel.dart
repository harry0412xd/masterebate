  ...
  String? importFromCsvString(String csvString) {
    ...
    for (var row in rows) {
      ...
      if (!inExpensesSection) {
        ...
        final headerKeywords = ['monthly', 'rebate', 'quota', 'cutoff', 'extra', 'amount', 'date', 'description', '%'];
        final looksLikeHeader = cleaned.any((c) => headerKeywords.any((k) => c.toLowerCase().contains(k)));
